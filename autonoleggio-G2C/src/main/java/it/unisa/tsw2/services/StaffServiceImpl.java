...
public class StaffServiceImpl implements StaffService
{
	...
	public List findByExample(Staff staff)
	{
		...
		try
		{
			...
			criteria.add( Example.create(staff).excludeZeroes().ignoreCase().enableLike(MatchMode.ANYWHERE) );
			list = criteria.list();
			session.getTransaction().commit();
		}
		catch(Exception e)
	...
	public boolean updateStaff(Staff staff)
	{
		...
		try
		{
			...
			session = util.getSessionFactory().getCurrentSession();
			session.beginTransaction();
			tmp_staff=(Staff)session.get(Staff.class, staff.getIdUtente());
			if(tmp_staff!=null)
			{
				tmp_staff.setCodicefiscale(staff.getCodicefiscale());
				tmp_staff.setUsername(staff.getUsername());
				tmp_staff.setPassword(staff.getPassword());
				tmp_staff.setIndirizzo(staff.getIndirizzo());
				tmp_staff.setCompetenza(staff.getCompetenza());
				tmp_staff.setCognome(staff.getCognome());
				tmp_staff.setNome(staff.getNome());
