...
public class OptionalServiceImpl implements OptionalService
{
	...
	public Optional findByNome_optional(String nome_optional)
	{
		...
		try
		{
			...
			criteria.add(Restrictions.eq("nome",nome_optional));
	...
	public boolean updateOptional(Optional optional)
	{
		...
		try
		{
			...
			session = util.getSessionFactory().getCurrentSession();
			session.beginTransaction();
			opt=(Optional)session.get(Optional.class, optional.getIdOptional());
			if(opt!=null)
			{
				opt.setNome(optional.getNome());
				opt.setDescrizione(optional.getDescrizione());
				opt.setSupplemento(optional.getSupplemento());
	...
	public Optional findById(Long id_optional)
	{
		...
		try
		{
			...
			Hibernate.initialize(optional.getVeicoli());
			session.getTransaction().commit();
		}
		catch(Exception e)
		{
