...
public class UtenteServiceImpl implements UtenteService
{
	...
	public boolean updateUtente(Utente utente)
	{
		...
		try
		{
			...
			session = util.getSessionFactory().getCurrentSession();
			session.beginTransaction();
			tmp_utente=(Utente)session.get(Utente.class, utente.getIdUtente());
			if(tmp_utente!=null)
			{
				tmp_utente.setCodicefiscale(utente.getCodicefiscale());
				tmp_utente.setUsername(utente.getUsername());
				tmp_utente.setPassword(utente.getPassword());
				tmp_utente.setIndirizzo(utente.getIndirizzo());
	...
	public Utente login(String username, String password) 
	{
		...
		try
		{
			...
			List<Utente> users = session.createQuery( "from Utente u where u.username='" + username + "'"
