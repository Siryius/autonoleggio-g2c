package it.unisa.tsw2.services;

import it.unisa.tsw2.domain.Sede;
import it.unisa.tsw2.domain.Veicolo;
import it.unisa.tsw2.util.HibernateUtil;

import java.util.List;

import org.hibernate.Criteria;
import org.hibernate.Hibernate;
import org.hibernate.Session;
import org.hibernate.criterion.Example;
import org.hibernate.criterion.MatchMode;
import org.hibernate.criterion.Property;
import org.hibernate.criterion.Restrictions;


public class VeicoloServiceImpl implements VeicoloService
{
	private Session session;
	private Criteria criteria;
	private HibernateUtil util;
	
	public VeicoloServiceImpl()
	{
		util=new HibernateUtil();
	}
	

	
	public Veicolo findByNome_veicolo(String nome_veicolo)
	{
		Veicolo veicolo=null;
		try
		{
					
			session = util.getSessionFactory().getCurrentSession();	
			session.beginTransaction();
			
			criteria= session.createCriteria(Veicolo.class);
			criteria.add(Restrictions.eq("nome_veicolo",nome_veicolo));
			veicolo=(Veicolo)criteria.uniqueResult();
			System.out.println("Ok query \n");
			
			session.getTransaction().commit();
		}
		catch(Exception e)
		{
			System.out.println("Errore VeicoloServiceImpl findByNome_veicolo");
			e.printStackTrace();
		}
		return veicolo;
	}
	
	/*public List findByCriteria(ArrayList criterias)
	{
		Optional optional=null;
		Criteria criteria;
		List list=null;
		try
		{
			if(criterias!=null)
			{
				session = util.getSessionFactory().getCurrentSession();	
				session.beginTransaction();
				criteria= session.createCriteria(Optional.class);
				for (int i=0;i<criterias.size();++i)
				{
					QueryCriteria criteria_query=(QueryCriteria)criterias.get(i);
					if(criteria_query.getAttributo().equals("supplemento_optional"))
					{
						
						float suppl_optional=Float.parseFloat(criteria_query.getValue());
						criteria.add(Restrictions.eq(criteria_query.getAttributo(),suppl_optional));
					}
					else
						criteria.add(Restrictions.eq(criteria_query.getAttributo(),criteria_query.getValue()));
				}	
				
				list=criteria.list();
				System.out.println("Ok query \n");
				session.getTransaction().commit();
				//System.out.println(optional.getDescrizione_optional());
				
			}
		}
		catch(Exception e)
		{
			System.out.println("Errore OptionalServiceImpl findByCriteria");
			e.printStackTrace();
		}
		return list;	
	}*/
	
	public List findByExample(Veicolo veicolo, String campoOrdinamento, String tipoOrdinamento)
	{
		List list=null;
		try
		{
			session = util.getSessionFactory().getCurrentSession();	
			session.beginTransaction();
			Criteria criteria = session.createCriteria(Veicolo.class);
			
			System.out.println("campoOrd "+campoOrdinamento);
			System.out.println("tipoOrd "+tipoOrdinamento);
			
			if(campoOrdinamento!=null && campoOrdinamento!="" )
			{
				System.out.println("1 campoOrd "+campoOrdinamento);
				System.out.println("1 tipoOrd "+tipoOrdinamento);
				
				if(tipoOrdinamento.equals("up"))
					criteria.add(Example.create(veicolo).excludeZeroes().ignoreCase().enableLike(MatchMode.ANYWHERE)).addOrder(Property.forName(campoOrdinamento).asc());
				else
					criteria.add(Example.create(veicolo).excludeZeroes().ignoreCase().enableLike(MatchMode.ANYWHERE)).addOrder(Property.forName(campoOrdinamento).desc());
			}
			list = criteria.list();
			session.getTransaction().commit();
		}
		catch(Exception e)
		{
			System.out.println("Errore VeicoloServiceImpl findByExample");
			e.printStackTrace();
		}
		return list;
	}

	public List findAllVeicolo()
	{
		List cri=null;
	
		try
		{
			session = util.getSessionFactory().getCurrentSession();
			session.beginTransaction(); 
			Criteria criteria = session.createCriteria(Veicolo.class);
			cri = criteria.list();
			session.getTransaction().commit();
		}
		catch(Exception e)
		{
			System.out.println("Errore VeicoloServiceImpl findAllVeicolo");
			e.printStackTrace();
		}
		return cri;
	}
	
	public boolean persist(Veicolo veicolo,String nome_veicolo)
	{
		boolean ris=true;
		try
		{
			Veicolo tmp_veicolo=null;
			tmp_veicolo=findByNome_veicolo(nome_veicolo);
			if(tmp_veicolo==null)
			{	
				session = util.getSessionFactory().getCurrentSession();
				session.beginTransaction();
				session.saveOrUpdate(veicolo);
				session.getTransaction().commit();
			}
			else
				ris=false;
		}
		catch(Exception e)
		{
			System.out.println("Errore VeicoloServiceImpl persist");
			e.printStackTrace();
		}
		return ris;
	}
	
	public boolean deleteVeicolo(Veicolo veicolo,String nome_veicolo)
	{
		boolean ris=true;
		try
		{

			Veicolo tmp_veicolo=this.findByNome_veicolo(nome_veicolo);
			if(tmp_veicolo!=null)
			{
				session = util.getSessionFactory().getCurrentSession();
				session.beginTransaction();
				session.delete(tmp_veicolo);
				session.getTransaction().commit();
			}
			else
				ris=false;
		}
		catch(Exception e)
		{
			System.out.println("Errore VeicoloServiceImpl deleteVeicolo");
			e.printStackTrace();
		}
	
		return ris;
	}
	
	public boolean updateVeicolo(Veicolo veicolo)
	{
		boolean ris=true;
		try
		{
			Veicolo tmp_veicolo=null;
			session = util.getSessionFactory().getCurrentSession();
			session.beginTransaction();
			tmp_veicolo=(Veicolo)session.get(Veicolo.class, veicolo.getIdVeicolo());
			if(tmp_veicolo!=null)
			{
				tmp_veicolo.setNome(veicolo.getNome());
				tmp_veicolo.setDescrizione(veicolo.getDescrizione());
				tmp_veicolo.setAlimentazione(veicolo.getAlimentazione());
				
				tmp_veicolo.setBagagliaio(veicolo.getBagagliaio());
				tmp_veicolo.setCambio(veicolo.getCambio());
				tmp_veicolo.setCategoria(veicolo.getCategoria());
				tmp_veicolo.setCilindrata(veicolo.getCilindrata());
				tmp_veicolo.setDisponibile(veicolo.getDisponibile());
				tmp_veicolo.setImmatricolazione(veicolo.getImmatricolazione());
				tmp_veicolo.setNposti(veicolo.getNposti());
				tmp_veicolo.setPortata(veicolo.getPortata());
				tmp_veicolo.setTarga(veicolo.getTarga());
				tmp_veicolo.setTipo(veicolo.getTipo());
				session.saveOrUpdate(tmp_veicolo);
			}
			else
				ris=false;
			
			session.getTransaction().commit();
		}
		catch(Exception e)
		{
			System.out.println("Errore VeicoloServiceImpl updateVeicolo");
			e.printStackTrace();
		}
		
		return ris;
	}
	
	public Veicolo findById(Long id_veicolo)
	{
		Veicolo veicolo=null;
		
		try
		{
			session = util.getSessionFactory().getCurrentSession();
			session.beginTransaction();
			veicolo=(Veicolo)session.get(Veicolo.class, id_veicolo);
			Hibernate.initialize(veicolo.getOptionals());
			Hibernate.initialize(veicolo.getPrenotazioni());
			Hibernate.initialize(veicolo.getPratiche());
			Hibernate.initialize(veicolo.getSede());
			session.getTransaction().commit();
		}
		catch(Exception e)
		{
			System.out.println("Errore VeicoloServiceImpl findById");
			e.printStackTrace();
		}
		
		return veicolo;
	}
}
