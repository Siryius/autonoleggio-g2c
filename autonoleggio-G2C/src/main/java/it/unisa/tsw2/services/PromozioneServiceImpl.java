...
public class PromozioneServiceImpl implements PromozioneService
{
	...
	public List findByExample(Promozione promozione)
	{
		...
		try
		{
			...
			criteria.add( Example.create(promozione).excludeZeroes().ignoreCase().enableLike(MatchMode.ANYWHERE) );
			list = criteria.list();
			session.getTransaction().commit();
		}
		catch(Exception e)
	...
	public boolean updatePromozione(Promozione promozione)
	{
		...
		try
		{
			...
			session = util.getSessionFactory().getCurrentSession();
			session.beginTransaction();
			tmp_pro=(Promozione)session.get(Promozione.class, promozione.getIdPromozione());
			if(tmp_pro!=null)
			{
				tmp_pro.setNome(promozione.getNome());
				tmp_pro.setDescrizione(promozione.getDescrizione());
				tmp_pro.setDataInizio(promozione.getDataInizio());
				tmp_pro.setDataFine(promozione.getDataFine());
				tmp_pro.setSconto(promozione.getSconto());
