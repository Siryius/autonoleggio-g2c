...
public class PraticaServiceImpl implements PraticaService
{
	...
	public boolean updatePratica(Pratica pratica)
	{
		...
		try
		{
			...
			session = util.getSessionFactory().getCurrentSession();
			session.beginTransaction();
			tmp_pratica=(Pratica)session.get(Pratica.class, pratica.getId_pratica());
			if(tmp_pratica!=null)
			{
				tmp_pratica.setNome_pratica(pratica.getNome_pratica());
				tmp_pratica.setDescrizione_pratica(pratica.getDescrizione_pratica());
				tmp_pratica.setScadenza_pratica(pratica.getScadenza_pratica());
